...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  /*
  override def mostRetweeted: Tweet =
    // use lazy evaluation, ensure there will be no exception caused by empty set
    lazy val leftMost = left.mostRetweeted
    lazy val rightMost = right.mostRetweeted

    val l = if left.isEmpty() then -1 else leftMost.retweets
    val r = if right.isEmpty() then -1 else rightMost.retweets


    val rt = elem.retweets
    if (rt > r && rt > l) then
      elem
    else if r > l then
      rightMost
  ...
  */
  ...
  override def mostRetweeted: Tweet =
    ...
    if !left.isEmpty() then
      ...
      if leftMost.retweets > res.retweets then
        res = leftMost

    if !right.isEmpty() then
      val rightMost = right.mostRetweeted
      if rightMost.retweets > res.retweets then
        res = rightMost
