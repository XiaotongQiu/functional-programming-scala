...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(e => e.toLower).map(e => (e._1, e._2.length)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match
    ...
    case x::xs =>
      for {
        z <- combinations(xs)
        n <- 0 to x._2
      } yield if n == 0 then z else (x._1, n)::z
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.toMap.foldLeft(x.toMap)((acc, occ) => {
      acc(occ._1) match
        case occ._2 => acc - occ._1
        case _ => acc.updated(occ._1, acc(occ._1)-occ._2)
    }).toList.sortBy {
      case (c, _) => c
    }
